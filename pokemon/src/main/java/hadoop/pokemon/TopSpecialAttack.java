...
public class TopSpecialAttack {
	...
	public static class PokemonMapper extends Mapper<LongWritable, Text, Text, LongWritable> {
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			context.write(new Text(strArr[1]), new LongWritable(Long.valueOf(strArr[8])));
		}
	...
	public static void main(String[] args) throws Exception {
		...
		Job job = Job.getInstance(conf, "TopAttackSpeed");
		job.setJarByClass(TopSpecialAttack.class);
		job.setMapperClass(PokemonMapper.class);
		job.setMapOutputKeyClass(Text.class);
		job.setMapOutputValueClass(LongWritable.class);
		job.setReducerClass(PokemonReducer.class);
		job.setNumReduceTasks(1);
		job.setOutputKeyClass(TopSpecialAttack.class);
